...
Помощник наград разрыва использует DBWIN как быстрый сигнал и независимое визуальное наблюдение как резерв. Перед OCR он проверяет только узкую область названий, русской моделью Tesseract распознаёт раскладки на 2/3/4 игрока и сопоставляет текст только с русскими названиями. Когда доступны загруженные пути реликвий отряда, поиск ограничивается их реальным пулом наград. `EE.log` остаётся последним отложенным резервом.
...
- [Мои предметы и продажа](docs/SELL_NOW.md)
