//! Парсер отряда из synqark/warframe-peer-overlay, 58d9497, Unlicense.
//! Используем события журнала; адреса остаются только в оперативной памяти.
...
impl LogParser {
    ...
    pub fn process_line(&mut self, line: &str) -> bool {
        ...
        if let Some(captures) = self.remove_squad.captures(line) {
            let (name, _) = parse_player_name(&captures[1]);
            self.loadout_bytes_by_name.remove(&name);
            self.ping_ips_by_name.remove(&name);
    ...
    fn reconcile(&mut self) -> bool {
        ...
        for peer in &mut self.peers {
            let ip = self
                ...
                .cloned();
            // Отсутствие адреса удалённого хоста не доказывает, что хост — мы.
...
mod tests {
    ...
    #[test]
    fn does_not_guess_local_host_when_addresses_are_missing() {
        let mut parser = LogParser::default();
