<script lang="ts">
  ...
  import { orderAdvice, orderMarketPrice } from "./marketSales";
  ...
  const change = (v: number | null | undefined) => v == null ? "—" : `${v > 0 ? "+" : ""}${number(v)}%`;
...
<div class="table-scroll" tabindex="0" role="region" aria-label="Объявления. В узком окне таблицу можно прокрутить горизонтально">
  <table>
    ...
    <thead><tr>
      ...
      <th><span>Ориентир</span><small>за штуку</small></th>
      <th aria-sort={sortAria("quantity")}><button class="sort" onclick={() => setSort("quantity")}>Кол-во {marker("quantity")}</button></th>
      <th class="trend" aria-sort={sortAria("trend")}><button class="sort" onclick={() => setSort("trend")}>Цена · 7 дней {marker("trend")}</button></th>
      <th class="trend" aria-sort={sortAria("volume")}><button class="sort" onclick={() => setSort("volume")}>Объём / день {marker("volume")}</button></th>
    ...
    <tbody>
      {#each displayed as row (row.order.id)}
        ...
        <tr class:attention={row.needsAction} class:hidden-order={!row.order.visible} class:chosen={selectedIds.has(row.order.id)}>
          ...
          <td class="numeric"><strong>{row.order.quantity}</strong>{#if row.order.type === "sell"}<small class:danger={row.health === "inventory_mismatch"} title="Свободно в вашем инвентаре">из {inventoryKnown ? row.inventory?.sellableQuantity ?? 0 : "?"} доступных</small>{/if}</td>
          <td class="trend"><button class="chart-link" aria-expanded={expanded === row.order.id} aria-label={"Статистика цены: " + name} title="Открыть графики и данные за период" onclick={() => expanded = expanded === row.order.id ? null : row.order.id}>
            {#if analyticsLoading && !data}<span class="muted">Загрузка…</span>{:else if data?.supported}<MarketTrendChart points={chartPoints(data)} compact/><span class="change" class:up={(data.priceChangePct ?? 0) > 0} class:down={(data.priceChangePct ?? 0) < 0}>{change(data.priceChangePct)}</span>{:else}<span class="muted">{analyticsError ? "Недоступно" : "Нет истории"}</span>{/if}
          </button></td>
          <td class="trend"><button class="chart-link" aria-expanded={expanded === row.order.id} aria-label={"Статистика объёма: " + name} onclick={() => expanded = expanded === row.order.id ? null : row.order.id}>
            {#if analyticsLoading && !data}<span class="muted">Загрузка…</span>{:else if data?.supported}<MarketTrendChart points={chartPoints(data)} metric="volume" compact/><span class="change">{number(data.current.dailyVolume)}<small>{data.current.dailyVolume === null ? `данные ${data.current.observedDays}/7 дней` : change(data.volumeChangePct)}</small></span>{:else}<span class="muted">{analyticsError ? "Недоступно" : "Нет истории"}</span>{/if}
          </button></td>
        ...
        {#if expanded === row.order.id}<tr class="expanded"><td colspan="8"><div class="expanded-content"><div class="detail-header"><strong>{name}</strong><button class="secondary" onclick={() => expanded = null}>Закрыть статистику</button></div>{#key row.key ? marketAnalyticsKey(row.key) : row.order.id}<MarketOrderInsight {row} summary={data ?? null}/>{/key}</div></td></tr>{/if}
      {/each}
    </tbody>
  </table>
</div>
...
<style>
  ...
  th.item { width:28%; font-weight:400; letter-spacing:0; text-transform:none; }
  th.trend,td.trend { width:10rem; } .actions-heading { width:7.5rem; }
  ...
  .change { font-weight:600; white-space:nowrap; font-variant-numeric:tabular-nums; } .change small { font-size:.75rem; white-space:nowrap; line-height:1.3; }
  ...
  .expanded > td { padding:.8rem; background:var(--surface-2); } .expanded-content { width:calc(100cqw - 1.6rem); max-width:100%; position:sticky; left:.8rem; } .detail-header { display:flex; align-items:center; justify-content:space-between; gap:1rem; margin-bottom:.75rem; }
  ...
  .pagination button,.detail-header button { min-height:1.9rem; padding:.3rem .6rem; font-size:.75rem; }
