<script lang="ts">
  ...
  const copy = {
    ru: {
      ...
      confirmedHint: "Сюда входят только доступные покупки с подтверждённой рыночной ценой.",
      ...
      methodologyBody: "Прямая продажа учитывается только при свежей цене и доступном количестве. Ассортимент Норы подтверждается во время обновления инвентаря из Warframe и учитывается до конца текущей ротации. Если получить его не удалось, вариант Ночной волны не входит в итог. Восфор считается по ожидаемой цене лучшего набора Лойда и показывается отдельно.",
    ...
    en: {
      ...
      confirmedHint: "Only affordable purchases with a confirmed market price are included.",
      expected: "plus about",
