<script lang="ts">
  ...
  import { filterMasteryItems, masteryCategoryLabel, masteryStatusLabel, masteryStore, type MasteryItemView, type MasteryStatus } from "./mastery";
  ...
  function explanation(item: MasteryItemView): string {
    ...
    if (item.reason === "no_record") return t("В полученной истории нет записи. Это не означает, что предмет не освоен.", "No entry was found in the received history. This does not mean the item is unmastered.");
    if (item.status === "progress") return t("По истории аккаунта максимальный ранг ещё не достигнут. Здесь показано освоение предмета за всё время, а не текущий ранг отдельной копии.", "Account history has not reached the maximum rank. This is lifetime mastery, not the current rank of an individual copy.");
    if (item.reason === "history_confirmed") return t("История аккаунта подтверждает максимальный ранг. Предмет остаётся освоенным после продажи или применения Формы.", "Account history confirms the maximum rank. Mastery remains after selling or applying Forma.");
    return t("Освоение подтверждено при сканировании снаряжения. Отметка сохраняется, даже если потом продать предмет или применить Форму.", "Mastery was confirmed by an equipment scan. This record remains even if the item is later sold or polarized.");
  }
</script>
...
<section class="mastery-screen" aria-labelledby="mastery-title">
  <header class="mastery-header">
    ...
    {#if view && view.catalogAvailable}
      <div class="history-status">
        ...
        <details><summary>{t("Что означают отметки", "About these labels")}</summary><p>{t("«Освоено» — достигнут максимальный ранг по истории аккаунта. «Не до конца» — есть прогресс, но максимальный ранг ещё не достигнут. «Нет данных» — нет записи либо правило пока не поддерживается. Продажа, применение Формы и настройка «Оставлять копий» не сбрасывают освоение. Для модульного снаряжения учитывается определяющая деталь, например призма усилителя, а не каждая сборка.", "Mastered means account history confirms the maximum rank. Incomplete means progress below the maximum rank. Unknown means a missing entry or unsupported rule. Selling, Forma and Keep copies do not reset mastery. Modular equipment is tracked by its mastery-bearing part, such as an amp prism, not each build.")}</p></details>
      </div>
    {/if}
  </header>
  ...
  {:else if view}
    <div class="mastery-filters">
      ...
      <label>{t("Освоение", "Mastery")}<select bind:value={status} onchange={() => page = 1}><option value="all">{t("Все состояния", "All statuses")}</option><option value="mastered">{t("Освоено", "Mastered")}</option><option value="progress">{t("Не до конца", "Incomplete")}</option><option value="unknown">{t("Нет данных", "Unknown")}</option></select></label>
    </div>
