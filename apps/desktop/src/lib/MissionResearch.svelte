<script lang="ts">
  ...
  import { MISSION_FILTERS, objectFilter, objectKindLabel, finitePosition, sceneBounds, fitCamera, followFrame, followTargetSignature, mapScale, worldToCanvas, zoomAt, scaledHeight, inHeightSlice, faceInHeightSlice, compareScenes, missionTime, missionBytes,
  ...
  $: visibleObjects = objects.filter(o => filters[objectFilter(o.kind)] && inHeightSlice(o.position[1], sliceEnabled, heightCenter, heightHalfWidth) && `${o.label} ${o.nameEn} ${o.itemPath ?? ""}`.toLocaleLowerCase("ru").includes(query.trim().toLocaleLowerCase("ru"))).sort((a, b) => (a.kind === "feather" ? 0 : 1) - (b.kind === "feather" ? 0 : 1) || a.label.localeCompare(b.label, "ru"));
  $: selected = objects.find(o => o.key === selectedKey) ?? null;
  $: difference = scene && previousScene ? compareScenes(previousScene, scene) : null;
  $: drawState = { scene, bounds, camera, objects: visibleObjects, selectedKey, sliceEnabled, heightCenter, heightHalfWidth, width: canvasWidth, height: canvasHeight };
  ...
  async function receive(next: MissionResearchStatus, version: number) {
    ...
    try {
      ...
      if (nextScene) {
        ...
        scene = nextScene;
        if (!continuingLive) {
          selectedKey = ""; followKey = ""; followHint = ""; listLimit = 100; query = "";
        ...
        scene = null; previousScene = null; selectedKey = ""; followKey = ""; followHint = ""; framingBounds = null; frameNextScene = true;
      }
  ...
  function pointerUp(event: PointerEvent) {
    ...
    if (nearest) { selectedKey = nearest.key; followKey = ""; followHint = ""; }
  }
  ...
  interface DrawState { scene: MissionScene | null; bounds: MapBounds; camera: MapCamera; objects: MissionObject[]; selectedKey: string; sliceEnabled: boolean; heightCenter: number; heightHalfWidth: number; width: number; height: number }
  function draw(node: HTMLCanvasElement, state: DrawState) {
    ...
    for (const object of state.objects) {
      ...
      const chosen = object.key === state.selectedKey; ctx.beginPath(); ctx.fillStyle = MISSION_FILTERS.find(f => f.key === objectFilter(object.kind))!.color;
  ...
  onMount(() => { void refresh(); void refreshArchives(); const timer = setInterval(() => void refresh(), 1000); return () => { alive = false; generation++; clearInterval(timer); }; });
</script>
...
<section class="mission" aria-label="Карта миссии">
  ...
  {#if scene}
    ...
    <div class="display-controls"><div class="filters" aria-label="Фильтры объектов">{#each MISSION_FILTERS as filter}<label><input type="checkbox" bind:checked={filters[filter.key]} /><i style:background={filter.color}></i>{filter.label}<span>{objects.filter(o => objectFilter(o.kind) === filter.key).length}</span></label>{/each}</div>
    ...
    <div class="explorer">
      <section class="map-panel" aria-label="Схема расположения объектов"><div class="map-tools"><span>Схема по прочитанным данным</span><div><button aria-label="Уменьшить карту" onclick={() => zoom(1 / 1.4)}>−</button><button aria-label="Увеличить карту" onclick={() => zoom(1.4)}>+</button><button onclick={resetMap}>Показать всё</button></div></div>
        ...
        <p class="map-help">Перетаскивайте схему и меняйте масштаб колёсиком. Светлые поверхности расположены выше. Выберите точку или объект в списке.</p>
      </section>
      <section class="object-panel" aria-label="Найденные объекты"><div class="list-heading"><h2>Объекты</h2><span>{visibleObjects.length} из {objects.length}</span></div><label class="search">Поиск объекта<input type="search" bind:value={query} placeholder="Название или тип" /></label>
        {#if !objects.length}<p class="empty">В этом чтении объекты не определены. Это не означает, что на миссии нет предметов.</p>{:else if !visibleObjects.length}<p class="empty">Нет объектов с выбранными фильтрами. Измените поиск, категории или диапазон высоты.</p>{:else}<ul class="object-list">{#each visibleObjects.slice(0, listLimit) as object (object.key)}<li><button class:selected={object.key === selectedKey} onclick={() => selectObject(object)}><i style:background={MISSION_FILTERS.find(f => f.key === objectFilter(object.kind))?.color}></i><span><strong>{object.label || objectKindLabel(object.kind)}</strong>{#if object.nameEn && object.nameEn !== object.label}<small>{object.nameEn}</small>{/if}<small>{objectKindLabel(object.kind)} · Высота {object.position[1].toFixed(1)}{object.positionFresh === false ? " · прежние координаты" : ""}</small></span></button></li>{/each}</ul>{#if visibleObjects.length > listLimit}<button class="more" onclick={() => listLimit += 100}>Показать ещё {Math.min(100, visibleObjects.length - listLimit)}</button>{/if}{/if}
      </section>
    </div>
