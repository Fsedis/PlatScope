<script lang="ts">
  ...
  import { MISSION_HIDDEN_STORAGE, indexHiddenMissionNames, objectIsHidden, parseHiddenMissionNames, serializeHiddenMissionNames, type HiddenMissionName } from "./missionFilters";
  ...
  function hideByName(object: MissionObject) {
    if (objectIsHidden(object, hiddenIndex)) return;
...
<section class="mission" aria-label="Карта миссии">
  ...
  <div class="explorer">
    ...
    <section class="map-panel" aria-label="Схема расположения объектов">
      ...
      {:else}<div class="initial"><div class="empty-map-icon">⌖</div><h2>{status?.busy ? "Строим карту миссии" : status?.autoStart ? status?.inOrbiter ? "В Орбитере карта не запускается" : "Карта запустится автоматически" : "Карта приостановлена"}</h2><p>{status?.busy ? "Находим предметы и проверяем их координаты. Результат появится здесь." : status?.gameRunning ? status?.autoStart ? status?.inOrbiter ? "После вылета и загрузки локации карта запустится автоматически." : "После загрузки локации найдём предметы и построим карту. Возвращение в Орбитер не запускает чтение." : "Нажмите «Запустить карту», чтобы возобновить карту и автоматический запуск в следующих локациях." : "Запустите Warframe и войдите на миссию. Здесь появятся предметы, тайники и ваш персонаж."}</p><span>Карта не записывает дампы на диск</span></div>{/if}
    </section>
...
<style>
  ...
  .object-list button.hide-name{flex:0 0 auto;margin-top:.55rem;padding:.3rem .2rem;font-size:.7rem;min-height:28px;color:var(--text-muted)}
</style>
