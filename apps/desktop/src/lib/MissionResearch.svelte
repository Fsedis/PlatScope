...
<section class="mission" aria-label="Карта миссии">
  ...
  <div class="explorer">
    ...
    <section class="map-panel" aria-label="Схема расположения объектов">
      ...
      {:else}<div class="initial"><div class="empty-map-icon">⌖</div><h2>{status?.busy ? "Строим карту миссии" : "Начните с карты"}</h2><p>{status?.busy ? "Находим предметы и проверяем их координаты. Результат появится здесь." : status?.gameRunning ? "Нажмите «Запустить карту» сверху. Затем выбирайте находки и собирайте свои фильтры." : "Запустите Warframe и войдите на миссию. Здесь появятся предметы, тайники и ваш персонаж."}</p><span>Карта не записывает дампы на диск</span></div>{/if}
    </section>
