    ...
    <aside class="relic-detail" aria-label={t("Выбранная реликвия","Selected relic") }>
      {#if selected}
        ...
        <section class="rewards"><h4>{t("Что может выпасть","Possible rewards")}</h4><div class="reward-labels"><span>{t("Награда","Reward")}</span><span>{scenario === "solo" ? t("Шанс","Chance") : t("Увидеть в выборе","In reward choices")}</span><span>{t("Цена ≈","Price ≈")}</span></div>
          {#each rewards as reward (reward.definition.rewardGameRef)}<div class="reward"><div><strong>{reward.displayName}</strong>{#each reward.targets as target (target.slug)}<button type="button" class="set-link" onclick={() => onPlanSet(target.slug)}>{target.finishes ? t("Завершит сет","Completes set") : t("Нужна для сета","Needed for set")}: {target.name} →</button>{/each}</div><span>{percent(rewardChoiceChance(reward.chance,scenario))}</span><span>{money(reward.price)}</span></div>{/each}
