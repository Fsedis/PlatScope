...
export function makeMissionScene(sequence = 1, source: "archive" | "live" = "archive"): MissionScene {
  ...
  // Первая дверь — координаты из записи 13.09; остальные варианты ключей — примеры интерфейса.
  for (const [index, code, keyName, english] of [[0,"health","здоровья","Bleeding"],[1,"shield","щитов","Decaying"],[2,"damage","урона","Extinguished"],[3,"speed","скорости","Hobbled"]] as const) {
    objects.push({key:`preview-dragon-${code}`,kind:"dragon_door",label:`Дверь Дракона · ключ ${keyName}`,nameEn:`Orokin Vault · ${english} Dragon Key`,itemPath:null,variantKey:`dragon-door-v1:${code}`,position:[208.74991+index*12,0.55,155.49994],typeNames:["ContextAction *"],availability:"unknown",details:[{label:"Требуемый ключ",value:`Ключ Дракона: ключ ${keyName} · ${english} Dragon Key`},{label:"Состояние двери",value:"Открытие двери и наличие ключа у игрока не проверены"}]});
  }
