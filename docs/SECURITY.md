...
Автообновление использует тот же read-only путь, включено по умолчанию и отключается в «Моих предметах». Локальное наблюдение `EE.log` не записывает файл и не управляет вводом. Ручные и фоновые чтения сериализованы; нет сетевого запроса инвентаря на каждом тике таймера. Ответ инвентаря ограничен 8 МиБ, ответы ротации/магазина — 4 МиБ, включая поток без `Content-Length`. Тело ответа сервера не включается в ошибку сканера. Кэш Норы и состояние повторов не содержат сессионных реквизитов.
