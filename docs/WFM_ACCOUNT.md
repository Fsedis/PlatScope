...
Переходы из PlatScope на страницы предметов всегда используют русскую локаль
Warframe Market: `https://warframe.market/ru/items/{slug}`.
...
Карточка кандидата Sell Now получает WFM `itemId` через локальный catalog join и ищет собственный sell-ордер по полному варианту: `itemId`, rank, subtype, `amberStars` и `cyanStars`. Это не позволяет показать или изменить ордер другого ранга как ордер выбранного предмета. В списке собственных ордеров русское имя дополняется английским названием WFM: это позволяет сразу сопоставить предмет с англоязычным сообщением покупателя. При английской локали или отсутствии перевода одинаковое имя повторно не показывается.
