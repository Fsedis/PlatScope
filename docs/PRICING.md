...
- **объём в день** — средний объём закрытых сделок Warframe Market за период, ориентир спроса;
...
В таблице объявлений раздела **Рынок** доступны отдельные сортировки по цене сделок и объёму. В **Моих предметах** список сортируется по названию, оценке цены, общему количеству или количеству к продаже; скрытой сортировки по очерёдности нет. Объём WFM не равен всем обменам в игре или достоверному числу проданных штук, а очерёдность не является прогнозом платины в день.
...
- актуальные предложения для выбранного варианта, если они уже загружены;
...
5. Rank, charges, subtype (включая refinement реликвии), amber/cyan stars и platform должны совпасть точно.
...
Свежий снимок резервного источника FrameForge с ценой, подтверждённой минимум тремя закрытыми сделками, сохраняет среднее доверие и участвует в расчёте докупки комплектов. Сам переход на резервный источник не должен обнулять раздел «Дособрать» и план перепродажи. Высокое доверие для резервного источника недоступно; старение данных, расхождение с текущими предложениями и недостаток подтверждённых сделок по-прежнему ограничивают рекомендации.
...
В **Моих предметах** предложения запрашиваются при выборе предмета, если нет ещё актуальной проверки его варианта, либо по кнопке **Обновить цены**. Цены продавцов показываются над формой объявления. Проверка ограничена выбранным вариантом; весь инвентарь автоматически не опрашивается. Срок действия ответа задаётся настройкой, по умолчанию — 90 секунд. При ошибке сохраняется ещё актуальный успешный ответ; устаревшая проверка не выдаётся за текущую.
...
Обычный Pricing Engine не оценивает конкретный мод Разлома с уникальным набором характеристик: возвращается причина `riven_pricing_unsupported`. Медианы обычных предметов и текущие предложения не используются для такой оценки. Данные о disposition оружия сами по себе не являются ценой конкретного мода.
