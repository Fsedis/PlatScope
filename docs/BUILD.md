...
`cargo test --workspace --all-features` включает проверку закрытия приложения на настоящих скрытых окнах Windows. Она не открывает рабочую БД, не подключается к игре и использует отдельный временный профиль WebView2. Главное окно должно завершать приложение даже при открытом скрытом оверлее; закрытие вспомогательного окна не должно завершать главное. Отдельный запуск: `cargo test -p platscope-desktop --test app_lifecycle`.
