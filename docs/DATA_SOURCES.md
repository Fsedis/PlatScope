...
```text
https://www.relics.run/history/item_data/items.json
```

Каталог хранит canonical `slug` как идентичность, английское имя и доступное `i18n.ru.name` отдельно. Локализованное имя влияет только на отображение и поиск, но никогда не заменяет ключ варианта.
